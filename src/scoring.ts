...
export async function processJob(supabase: any, job: any): Promise<void> {
  ...
  try {
    const scorePercentage = maxPossible > 0 ? Math.round((overallScore / maxPossible) * 100) : 0;
    const grade = scorePercentage >= 80 ? 'A' : scorePercentage >= 65 ? 'B' : scorePercentage >= 50 ? 'C' : scorePercentage >= 35 ? 'D' : 'F';
    ...
    const takeaway = dealKiller ? `Deal killer: ${typeof dealKiller === 'string' ? dealKiller : dealKiller.summary || JSON.stringify(dealKiller)}` : topFix ? `Top fix: ${typeof topFix === 'string' ? topFix : topFix.issue || JSON.stringify(topFix)}` : '';
...
- filler_word_count and strategic_pauses MUST be plain integers, not strings.`;
  ...
  try {
    const resp = await undiciRequest(
    ...
    if (resp.statusCode !== 200) {
      ...
      console.error('Gemini generateContent error:', resp.statusCode, errBody);
      return null;
    }
  ...
  if (provider === 'google') {
    const resp = await undiciRequest(
      `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        headersTimeout: GENERATE_CONTENT_TIMEOUT_MS,  // 10 min
        bodyTimeout: GENERATE_CONTENT_TIMEOUT_MS,      // 10 min
        body: JSON.stringify({
          systemInstruction: { parts: [{ text: systemPrompt }] },
    ...
    if (resp.statusCode !== 200) throw new Error(`Gemini error: ${resp.statusCode} ${await resp.body.text()}`);
    const data = await resp.body.json() as any;
...
function splitPlainStringToIssueFix(text: string): { issue: string; fix: string; script_reference: string | null } {
  ...
  // No separator found — treat entire string as issue
