 ...
 * 
 * FIXES APPLIED:
 * 1. safeParseJSON: improved truncation repair (handles unclosed strings in arrays/objects)
 * 2. Token limits set to 16384 for all providers
 * 3. Increased Gemini ACTIVE wait and fetch timeouts via scoring.ts
 */
...
// ============================================================
// SAFE JSON PARSE with repair for truncated LLM output
// ============================================================
...
function safeParseJSON(text: string): any {
  ...
  // Strip leading markdown headers (e.g., "# Sales Analysis\n")
  ...
  // Repair attempt: fix truncated JSON
  try {
    ...
    const cutPoints = [lastCompleteComma, lastCompleteBrace, lastCompleteBracket].filter(p => p > 0);

    if (cutPoints.length > 0) {
      const bestCut = Math.max(...cutPoints);

      // If the text after the cut point looks incomplete, truncate there
      const afterCut = repaired.substring(bestCut);
      if (afterCut.includes('"') && (afterCut.split('"').length % 2 === 0)) {
        // Odd number of quotes after cut = unclosed string
        repaired = repaired.substring(0, bestCut);
    ...
    const bracketDiff = openBrackets - (repaired.match(/]/g) || []).length;
    const braceDiff = openBraces - (repaired.match(/}/g) || []).length;

    // We need to close inner structures first (arrays before objects)
    // Simple heuristic: close brackets then braces
    ...
    console.error(`JSON repair also failed. First 500 chars: ${cleaned.substring(0, 500)}`);
    throw new SyntaxError(`JSON parse and repair both failed: ${(e2 as Error).message}`);
  }
}
  ...
  try {
    ...
    if (isActive) {
      ...
      try {
        ...
        audioDelivery = safeParseJSON(audioResult);
    ...
    // 5. Score transcript with Claude/OpenAI (with audio context if available)
    const scoringResult = await scoreTranscript(
      transcript,
      rubric,
      call_context,
      audioDelivery,
...
// ============================================================
// SCORE TRANSCRIPT (Claude / OpenAI / OpenRouter)
// ============================================================
...
async function scoreTranscript(
  ...
  const userPrompt = `Score this sales call transcript:\n\n${transcript}`;
...
// ============================================================
// PROMPT BUILDERS (stubs — replace with your actual prompts)
// ============================================================
